/* =============================================
   HEALTHY BEING – PREMIUM JAVASCRIPT ENGINE
...
function showCookieBar() {
  if (localStorage.getItem('hb_cookie_accepted')) return;
  const bar = document.getElementById('cookie-bar');
  if (bar) setTimeout(() => bar.classList.add('show'), 800);
}
document.getElementById('cookieAccept')?.addEventListener('click', () => {
  localStorage.setItem('hb_cookie_accepted', 'true');
...
if (dot && ring && !isTouchDevice && !prefersReducedMotion && window.innerWidth > 768) {
  ...
  const hoverTargets = 'a, button, .service-card, .team-card, .faq-q, .symptom-chip, .stat-box, .testi-card, .award-card, .btn-primary, .btn-hero-primary, .btn-hero-secondary, .cond-item, input, textarea, select, label';
...
(function initParticles() {
  ...
  class Particle {
    ...
    reset() {
      ...
      this.color = Math.random() > 0.6 ? '#2DD4A0' : '#ffffff';
    }
    update() {
    ...
    for (let i = 0; i < particles.length; i++) {
      for (let j = i + 1; j < particles.length; j++) {
        ...
        if (dist < 100) {
          ...
          ctx.strokeStyle = isDark ? 'rgba(45,212,160,' + (0.12 * (1 - dist/100)) + ')' : 'rgba(26,157,111,' + (0.06 * (1 - dist/100)) + ')';
          ctx.lineWidth = 0.5;
          ctx.stroke();
        }
      }
...
langBtn?.addEventListener('click', () => {
  ...
  localStorage.setItem('hb_lang', currentLang);
...
function initReveal() {
  ...
  document.querySelectorAll('.service-card, .stat-box, .testi-card, .team-card, .award-card, .faq-item').forEach((el, i) => {
...
function initHeroParallax() {
  ...
  if (!hero || prefersReducedMotion || isTouchDevice) return;
  window.addEventListener('scroll', () => {
    const y = window.scrollY;
    if (y < window.innerHeight) {
      hero.style.transform = `translateY(${y * 0.15}px)`;
      hero.style.opacity = 1 - (y / window.innerHeight) * 0.3;
    }
  }, { passive: true });
}
...
console.log('%cHealthy Being', 'color:#1A9D6F;font-size:22px;font-weight:700;letter-spacing:-0.5px;');
console.log('%cPremium Wellness Experience', 'color:#5A7066;font-size:11px;');
